...
export function WoundCareSupport({ vertical }: WoundCareSupportProps) {
  return (
    <WoundSection className="py-9 xs:py-12" id="wound-support">
      <WoundBackgroundImage className="-right-16 top-2 h-[18rem] w-[34rem] opacity-10 sm:h-[24rem] sm:w-[48rem] sm:opacity-12" src="/wound-care/amber-wave.png" />
      <WoundContainer>
        <WoundReveal>
          <WoundKicker>Care support</WoundKicker>
          <h2 className="mt-4 font-heading text-2xl font-bold text-primary-dark xs:text-3xl">How our wound-care support works</h2>
          <p className="sr-only">{vertical.name}</p>
        </WoundReveal>
        <div className="mt-7 grid gap-7 lg:grid-cols-[0.28fr_0.44fr_0.28fr]">
          <WoundReveal>
            <div className="relative min-h-[220px] overflow-hidden rounded-[1.1rem] bg-[#f4faff] xs:min-h-[260px] sm:min-h-[300px]">
              <WoundAsset
          ...
          <WoundReveal className="grid gap-4" delay={0.08}>
            {supportSteps.map((step, index) => {
              ...
              return (
                <div className="grid grid-cols-[auto_1fr] items-center gap-3 rounded-[1rem] bg-white/96 px-4 py-4 shadow-[0_14px_38px_rgba(16,42,67,0.07)] ring-1 ring-[#c9e6f8] xs:grid-cols-[auto_1fr_auto] xs:gap-4 xs:px-5" key={step.title}>
                  ...
                  <Icon aria-hidden="true" className="hidden h-7 w-7 text-primary/70 xs:block" />
                </div>
              );
            })}
          </WoundReveal>
          ...
          <WoundReveal delay={0.14}>
            <div className="h-full rounded-[1.1rem] bg-[linear-gradient(135deg,#fff7e8,#ffffff_62%,#fff2dd)] p-4 shadow-[18px_24px_65px_rgba(245,166,35,0.12)] ring-1 ring-[#efd8b3] xs:p-6 sm:rounded-[1.25rem]">
